...
RELEASE_BASE_URL="https://github.com/${GITHUB_REPO}/releases/download/v${BETA_VERSION}"

# 下载文件名
LINUX_X64_URL="${RELEASE_BASE_URL}/OpenClaw-${BETA_VERSION}-linux-x64.tar.gz"
LINUX_ARM64_URL="${RELEASE_BASE_URL}/OpenClaw-${BETA_VERSION}-linux-arm64.tar.gz"
MACOS_X64_URL="${RELEASE_BASE_URL}/OpenClaw-${BETA_VERSION}-macos-x64.tar.gz"
MACOS_ARM64_URL="${RELEASE_BASE_URL}/OpenClaw-${BETA_VERSION}-macos-arm64.tar.gz"
# ===================================
...
get_download_url() {
    ...
    if [[ "$os" == "linux" ]]; then
        if [[ "$arch" == "arm64" ]]; then
            echo "$LINUX_ARM64_URL"
        else
            echo "$LINUX_X64_URL"
        fi
    elif [[ "$os" == "macos" ]]; then
        if [[ "$arch" == "arm64" ]]; then
            echo "$MACOS_ARM64_URL"
        else
            echo "$MACOS_X64_URL"
        fi
    else
...
main() {
    ...
    local archive_name="openclaw-beta.tar.gz"
    local archive_path="$tmpdir/$archive_name"
    
    # 下载
    echo -e "${INFO}Downloading...${NC}"
