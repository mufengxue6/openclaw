...
注意: replace `18789` with your configured `gateway.port` (or `--port`/`OPENCLAW_GATEWAY_PORT`).
注意: when you pass `--url`, the CLI does not fall back to config or environment credentials.
