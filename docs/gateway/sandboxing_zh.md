...
- `"all"`: every session runs in a sandbox.
  注意: `"non-main"` is based on `session.mainKey` (default `"main"`), not agent id.
...
注意: the default image does **not** include Node. If a skill needs Node (or
