...
注意: The gateway runs **directly on the host** (not in Docker), but agent sandboxes use Docker for isolation. See [Sandboxing](/gateway/sandboxing) for details.
...
注意: This is idempotent and safe to run multiple times.
