...
**注意:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/openclaw.json`.

**注意:** The Discord token can come from either:
...
**注意:** 512MB is too small. 1GB may work but can OOM under load or with verbose logging. **2GB is recommended.**
...
**注意:** `fly sftp` may fail if the file already exists. Delete first:
...
**注意:** After `fly deploy`, the machine command may reset to what's in `fly.toml`. If you made manual changes, re-apply them after deploy.
